...
library(admixtools, lib.loc="/dss/dsshome1/lxc0E/di67kah/R")
library(tidyverse)
library(magrittr)
library(dplyr)
library(parallel)
...
pop = c("SPA1k","SPA0","EURw0","EURc0","EURc2k","EURc5k","EURnc1k", 
        "IRQ0","EURse0","EURe2k","EURse1k","EURse2k","EURse10k","EURse20k","JACKDAW","MON","AM")
...
# set4: compare EURw to all hoodies across time using SPA as pop1
...
d1 <- ggplot(f4st) +
  ...
  scale_shape_manual(values = c("SPA1k"=21,"EURw0"=3,"EURc0"=4,"EURnc1k"=21, "EURnw2k"=22, "EURw2k"=22, "EURc2k"=22,"EURc6k"=23)) +
  labs(x="f4-estimates (all) ", y="Hooded crow") +
  guides(shape=guide_legend("Carrion crow", override.aes = list(shape = c(3,4,21,21,22,22,22,23), fill=c("white","white","#7F3B08","#FEE0B6","#E08214","#B35806","#FDB863","#FDB863"))), 
...
#Heatmap matrix
md2 <- ggplot(f4st, aes(x = pop2, y = pop4, fill = est, size=est)) +
  geom_point(shape = 21) +
...
e1 <- ggplot(f4st%>%filter(pop4!="JACKDAW")) +
  ...
  labs(x="f4-estimates (all)", y="Hooded crow") +
  guides(shape=guide_legend("Carrion crow", override.aes = list(shape = c(10,3,4,21,21,22,22,22,23), fill=c("white","white","white","#7F3B08","#FEE0B6","#E08214","#B35806","#FDB863","#FDB863"))), 
...
plot_admixture_map <- function(df, title) {
  ggplot() +
    ...
    coord_sf(xlim = c(-15, 65), ylim = c(30, 65), expand = FALSE) +
    scale_fill_manual(values = c("carrion" = "black", "hooded" = "white")) +
    theme_minimal() + annotate("text",x = -10, y = 63, label = title, hjust = 0, size = 4) + labs(fill = "Ancestry") +
...
admmap <- plot_spacer() + plot_spacer() + map0 + map1 + map2 + map5 + map10 + map20 +
  plot_layout(ncol=3) &
  theme(legend.position = "top", plot.margin = unit(c(0, 0, 0, 0), "pt")) #6x8, edit size of piecharts later
...
# -----------------------------------------------------------------------------------------------------------------------------------------------------
# specific check of excess admixture ------------------------------------------------------------------------------------------------------------------
f4(f2_blocks, "SPA0", "EURc0", "EURse10k","EURse0")
f4(f2_blocks, "SPA0", "EURc0", "EURse20k","EURse0")
f4(f2_blocks, "SPA0", "EURc0", "EURse20k","EURse10k")

f4(f2_blocks_trans, "SPA0", "EURc0", "EURse10k","EURse0")
f4(f2_blocks_trans, "SPA0", "EURc0", "EURse20k","EURse0")
f4(f2_blocks_trans, "SPA0", "EURc0", "EURse20k","EURse10k")

f4(f2_blocks_trans, "MON", "AM", "EURse10k","EURse0") # false positives
f4(f2_blocks_trans, "MON", "AM", "EURse20k","EURse0")
f4(f2_blocks_trans, "MON", "AM", "EURse20k","EURse10k")
